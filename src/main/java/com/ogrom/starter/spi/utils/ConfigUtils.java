...
public class ConfigUtils {
    ...
    public static List<String> mergeValues(Class<?> type, String cfg, List<String> def) {
        ...
        // add initial values
        String[] configs = (cfg == null || cfg.trim().length() == 0) ? new String[0] : CommonConstants.COMMA_SPLIT_PATTERN.split(cfg);
        ...
        if (!names.contains(CommonConstants.REMOVE_VALUE_PREFIX + CommonConstants.DEFAULT_KEY)) {
            ...
            int i = names.indexOf(CommonConstants.DEFAULT_KEY);
            if (i > 0) {
                names.addAll(i, defaults);
            } else {
                names.addAll(0, defaults);
            }
            names.remove(CommonConstants.DEFAULT_KEY);
        } else {
            names.remove(CommonConstants.DEFAULT_KEY);
        }
